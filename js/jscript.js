  ...
  init();
   ...
   setInterval(function(){
    ...
    $(".left-menu li a").on("click",function(e){
        ...
        if($(this).hasClass("parent-menu")) return false;
        
          
         var id = $(this).attr("href");
        
         $(".left-menu li a").removeClass("active_link_btn");
         $(".left-menu li").removeClass("active_link_btn");
      
         var catalog_id =   $(this).attr("href");
         $(this).parent().addClass("active_link_btn");
          $(this).addClass("active_link_btn");
          
          get_content(id,"get_catalog");
...
function get_content(id,evet_msg){
    
   
    var loading_bar ='<center><h4>Loading...</h4></center><div class="progress progress-striped active"><div data-original-title="95%" class="bar tipb" style="width: 100%;"></div></div>';
                  
     $.ajax({
            url: "ajax.php?class=fontend&event_msg="+evet_msg,
            type: "POST",
            data: {data_id:id},
            beforeSend: function(xhr) {
                xhr.overrideMimeType("text/plain; charset=x-user-defined");
               if( evet_msg != "get_product_detail")
                 $("#main-content").empty().html(loading_bar);
            }
        }).done(function(data) {
           // $("#msg_popup_modal_content").empty().html(data);
      
       if( evet_msg == "get_product_detail")
           $("#show-content").empty().hide().html(data).fadeIn();
          else {
           $("#main-content").empty().hide().html(data).fadeIn();
            $('.fadein img:gt(0)').hide();
            $('#myCarousel').carousel();
            init();
          }
...
{   
        ...
        $(".product-menu-list a").on("click",function(e){
            e.preventDefault();
          
           $(".product-menu-list a").removeClass("active_link_btn");
           $(".product-menu-list").removeClass("active_link_btn");
           var product_id = $(this).attr("href");
            $(this).parent().addClass("active_link_btn");
           $(this).addClass("active_link_btn");
          get_content(product_id ,"get_product_detail");
